...
for (i in 1:length(chr)) {
	filenames[i] <- paste(paste(tag, "plusP", sep="_"), "plus", chr[i], "CSARNhits", sep=".")
...
c2 <- vector("integer",length(chr))
for (i in 1:length(chr)) {
	filenames[i] <- paste(paste(tag, "plusP", sep="_"), "minus", chr[i], "CSARNhits", sep=".")
	values <- LoadBinCSAR(filenames[i])
	chrL_0[i] <- length(which(values>0))
	c1[i] <- sum(as.numeric(values))
	c2[i] <- sum(as.numeric(values)^2)
...
c2 <- vector("integer",length(chr))
for (i in 1:length(chr)) {
	filenames[i] <- paste(paste(tag, "minusP", sep="_"), "plus", chr[i], "CSARNhits", sep=".")
	values <- LoadBinCSAR(filenames[i])
	chrL_0[i] <- length(which(values>0))
	c1[i] <- sum(as.numeric(values))
	c2[i] <- sum(as.numeric(values)^2)
...
c2 <- vector("integer",length(chr))
for (i in 1:length(chr)) {
	filenames[i] <- paste(paste(tag, "minusP", sep="_"), "minus", chr[i], "CSARNhits", sep=".")
	values <- LoadBinCSAR(filenames[i])
	chrL_0[i] <- length(which(values>0))
	c1[i] <- sum(as.numeric(values))
	c2[i] <- sum(as.numeric(values)^2)
...
# run CSAR
score.forward <- ChIPseqScore(control=nhitsC.forward,sample=nhitsS.forward,file=sprintf("%s.plus",tag),times=10000,test="Poisson")
score.reverse <- ChIPseqScore(control=nhitsC.reverse,sample=nhitsS.reverse,file=sprintf("%s.minus",tag),times=10000,test="Poisson")
...
results$start <- start(ranges(win))
